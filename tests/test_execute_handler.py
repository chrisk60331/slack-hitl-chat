...
class TestExecuteHandler:
    ...
    async def test_create_ai_agent_missing_token(self) -> None:
        ...
        with pytest.raises(
            ValueError, match="MCP_AUTH_TOKEN environment variable is required"
        ):
    ...
    async def test_execute_action_from_text_success(
        self, mock_create_agent: AsyncMock, mock_execute_action: AsyncMock
    ) -> None:
    ...
    async def test_execute_action_from_text_timeout(
        self, mock_create_agent: AsyncMock, mock_wait_for: AsyncMock
    ) -> None:
        ...
        result = await execute_action_from_text(
            "Create user test@example.com", timeout_seconds=5
        )
    ...
    def test_lambda_handler_string_body(self, mock_execute: AsyncMock) -> None:
        ...
        mock_result = ExecutionResult(
            request_id="direct_execution", execution_status="success"
        )
        mock_execute.return_value = mock_result

        event = {"body": json.dumps({"action_text": "List all users"})}
    ...
    @patch("src.execute_handler.execute_action_from_text")
    def test_lambda_handler_with_request_id_lookup(
        self, mock_execute: AsyncMock, mock_get_approval: Mock
    ) -> None:
        ...
        mock_result = ExecutionResult(
            request_id="test-request-123", execution_status="success"
        )
        mock_execute.return_value = mock_result

        event = {"request_id": "test-request-123", "execution_timeout": 300}
    ...
    def test_lambda_handler_request_id_not_found(self, mock_get_approval: Mock) -> None:
        ...
        event = {"request_id": "nonexistent-request"}
