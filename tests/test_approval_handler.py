...
class TestHandleApprovalDecision:
    ...
    def test_handle_approval_decision_not_found(self) -> None:
        ...
        with patch('src.approval_handler.table', table):
            with pytest.raises(ValueError, match="Request ID nonexistent-123 not found"):
...
class TestLambdaHandler:
    ...
    def test_lambda_handler_error(self) -> None:
        ...
        event = {
            'body': 'invalid json that will cause an error'
        }
        
        with patch('src.approval_handler._extract_decision_data', side_effect=Exception("Test error")):
            result = lambda_handler(event, {})
...
class TestSendNotifications:
    ...
    @patch('src.approval_handler.LAMBDA_FUNCTION_URL', 'https://test.lambda-url.us-east-1.on.aws/')
    def test_send_notifications_pending_with_links(self, mock_sns: Mock) -> None:
        ...
        assert 'https://test.lambda-url.us-east-1.on.aws/?request_id=test-123&action=approve' in message
        assert 'https://test.lambda-url.us-east-1.on.aws/?request_id=test-123&action=reject' in message
