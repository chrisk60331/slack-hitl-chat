...
class AgentOrchestrator:
    ...
    def __init__(self, memory: Optional[ShortTermMemory] = None, policy_engine: Optional[PolicyEngine] = None) -> None:
        self.memory = memory or ShortTermMemory()
        self.policy_engine = policy_engine or PolicyEngine()
        self._sfn_client = boto3.client("stepfunctions", region_name=os.getenv("AWS_REGION"))
    ...
    async def run(self, request: OrchestratorRequest) -> OrchestratorResult:
        ...
        if not request.category or not request.resource:
            inferred_category, inferred_resource = infer_category_and_resource(request.query)

        proposed = ProposedAction(
            tool_name=request.tool_name or "auto",
            description=request.query,
            category=self._coerce_category(request.category) if request.category else inferred_category,
    ...
    async def _execute_direct(self, request: OrchestratorRequest) -> OrchestratorResult:
        ...
        full_query = f"{prompt_prefix}\n\nUser request: {request.query}" if prompt_prefix else request.query
    ...
    def _wait_for_approval(self, request_id: str, timeout_seconds: int = 1800, poll_interval: int = 10) -> str:
    ...
    @staticmethod
    def _coerce_category(category: Optional[str]):
